...
my_goal = [3000,0]; %coordinates of goal (adjustable)
dc = 5750.0; %cut off distance
rho = 250.0; %attractive force threshold distance/min distance to goal
zeta = .7; %weight of attractive force (adjustable)
eta = 7e7; %weight of repulsive force 9e7 (adjustable)
gain_tvel = 1; %translational velocity gain (adjustable)
gain_svel = 10.0; %rotational velocity gain(adjustable)
...
minSonarID = 8;    % this has the sonar with the min reading
minRange = 3*MIN_CLEARANCE;  % initialize to something
phi = [90 50 30 10 -10 -30 -50 -90 -90 -130 -150 -170 170 150 130 90] * pi/180; %angle of each sonar in radians
...
while(p3_bumpersClear(numFrontBumpers, numRearBumpers) && rangeClearance)
    ...
    for ix =1:numSonars
        if(sonarRanges(ix) <= dc)
            temp = -1 * eta * (1/sonarRanges(ix)- 1/dc) * 1/sonarRanges(ix);
        else
            temp = 0.0;
        end
